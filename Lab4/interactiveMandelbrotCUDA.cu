...
// Image data
	...
	unsigned char   *pixels_gpu;

	__managed__ // Memory accessible both from CPU and GPU.
...
struct cuComplex
{
	...
	__device__ //__device__ is local to the GPU
...
void Draw()
{
	...
	cudaMalloc( (void**)&pixels_gpu, gImageHeight*gImageWidth*4 );
	cudaMemcpy( pixels_gpu, pixels, gImageHeight*gImageWidth*4, cudaMemcpyHostToDevice);
	...
	computeFractal<<<numBlocks, threadsPerBlock>>>(pixels_gpu);
	...
	cudaMemcpy( pixels, pixels_gpu, gImageHeight*gImageWidth*sizeof(unsigned char)*4, cudaMemcpyDeviceToHost );
